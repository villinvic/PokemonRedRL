...
class GoExplorePokemon:
    ...
    def __init__(self, environment, path: Path, relevant_state_features,
                 sample_base_state_chance=0.45, recompute_score_freq=1, rendering=False):
    ...
    def add_starting_point(self, game_stats):
        ...
        if np.random.random() < 1e-1 or state_name not in files:
    ...
    def sample_starting_point(self, scores):
        ...
        if np.random.random() < self.sample_base_state_chance or len(self.states) == 0:

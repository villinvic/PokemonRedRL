...
class PokemonICMModel(TFModelV2):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        self.curiosity_reward_scale = model_config.get("icm_eta", 0.1) / 2.
        ...
        if self.learner_bound:
            ...
            last_layer = next_screen_input

            for cnn_layer in cnn_layers:
                last_layer = cnn_layer(last_layer)

            next_state_pre_f1 = state_embedding_concat(
                [last_layer, next_stats_input]
            )
    ...
    def state_prediction_loss(self):
        return tf.math.square(tf.squeeze(self.icm_next_state_embedding - self.icm_state_predictions))

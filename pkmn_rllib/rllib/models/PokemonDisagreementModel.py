...
class PokemonDisagreementMModel(TFModelV2):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        if self.learner_bound:
            ...
            curr_screen_input = tf.keras.layers.Input(shape=(36*2,) + obs_space["screen"].shape[1:], name="curr_screen_input", #(36,) + obs_space["screen"].shape[1:]
                                                      dtype=tf.float32)
            next_screen_input = tf.keras.layers.Input(shape=(36*2,) + obs_space["screen"].shape[1:], name="next_screen_input",
                                                      dtype=tf.float32)
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        if self.learner_bound:
            ...
            self.clipped_curr_screen = self.screen_input[:, :36*2]
            self.clipped_next_screen = next_screen_input[:, :36*2]
